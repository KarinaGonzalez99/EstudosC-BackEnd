...
namespace AwaitCatchFinally
{
    class Programa
    {
		// Método Main, ponto de entrada do programa, é assíncrono
		public async void Main()
        {
            WriteLine("9. Await Em Blocos Catch E Finally");

			// Cria um StreamWriter para registrar logs em um arquivo de texto
			StreamWriter logAplicacao = new StreamWriter("LogAplicacao.txt");

            try
            {
				// Escreve uma linha no log de forma assíncrona
				...
				// Registra a criação do aluno no log
				...
				// Registra a criação do segundo aluno no log
            ...
            {
				// Captura exceção de argumento específico e registra no log
            ...
            {
				// Captura outras exceções de argumento e registra no log
            ...
            {
				// Captura qualquer outra exceção e registra no log
				await logAplicacao.WriteLineAsync(exc.ToString());
                Console.WriteLine(exc.ToString());
            }
            finally
            {
				// Executa código de limpeza, registrando o final da aplicação no log e liberando o recurso
